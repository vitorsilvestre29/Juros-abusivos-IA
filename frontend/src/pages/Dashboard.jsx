    ...
    <div style={{ minHeight: '100vh', background: bg, fontFamily: sans }}>
      <nav style={{ background: N, boxShadow: '0 2px 12px rgba(13,33,55,0.25)', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            <button onClick={logout} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.12)', color: '#7E9BB5', borderRadius: 8, padding: '8px 16px', fontSize: 13, cursor: 'pointer', fontFamily: sans }}>
              Sair
            </button>
          </div>
        </div>
      ...
      <main style={{ maxWidth: 960, margin: '0 auto', padding: '52px 24px' }}>
        <div style={{ marginBottom: 44 }}>
          <h1 style={{ fontFamily: serif, fontSize: 34, fontWeight: 700, color: N, marginBottom: 6 }}>Minhas analises</h1>
        ...
        {loading === false && history.length === 0 && (
          <div style={{ background: white, border: '1px solid ' + border, borderRadius: 20, padding: '72px 32px', textAlign: 'center', boxShadow: '0 2px 16px rgba(13,33,55,0.05)' }}>
        ...
        {loading === false && history.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {history.map(item => (
              <div key={item.contract_id} style={{ background: white, border: '1px solid ' + border, borderRadius: 18, padding: '24px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, boxShadow: '0 2px 8px rgba(13,33,55,0.04)' }}>
                ...
                {item.impact_brl > 0 && (
                  <div style={{ textAlign: 'right', background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 12, padding: '10px 16px' }}>
                ...
                <div>
                  {item.analysis_status === 'completed' && item.paid === false && (
                    <button onClick={() => nav('/analise/' + item.contract_id)} style={{ background: N, color: white, border: 'none', borderRadius: 10, padding: '10px 22px', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: sans }}>
                      Ver resultado
                    </button>
                  )}
                  {item.paid && item.analysis_id && (
                    <button onClick={() => nav('/laudo/' + item.analysis_id)} style={{ background: '#15803D', color: white, border: 'none', borderRadius: 10, padding: '10px 22px', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: sans }}>
                      Ver laudo
                    </button>
                  )}
                  {(item.analysis_status === 'pending' || item.analysis_status === 'processing') && (
                    <button onClick={() => nav('/analise/' + item.contract_id)} style={{ background: bg, color: muted, border: '1px solid ' + border, borderRadius: 10, padding: '10px 22px', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: sans }}>
                      Acompanhar
                    </button>
                  )}
                </div>
